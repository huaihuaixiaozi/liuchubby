...
| Live trace, LCP attribution, INP attribution, layout shift attribution | Chrome DevTools MCP server (`performance_*`, `lighthouse_audit`) | `chrome-devtools` MCP server configured in the harness (see `browser-testing-with-devtools`) |
...
9. Delegate granular optimization guidance and remediation steps to `performance-optimization` — keep this report at the audit level.
...
- **Do not invoke from another persona.** If `code-reviewer` flags a performance concern that warrants a deeper pass, surface that recommendation in the report; the user or a slash command initiates the deeper pass. See the shipping-and-launch skill for orchestration rules.
